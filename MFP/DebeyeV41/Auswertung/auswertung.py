...
d = lam/(2*np.sin(PeakAngle/2 * np.pi / 180))

# Compute possible millerindizes for fcc and bcc
h_bcc, k_bcc, l_bcc = lat.bcc(7)
h_fcc, k_fcc, l_fcc = lat.fcc(7)

n_bcc = np.sqrt(h_bcc**2 + k_bcc**2 + l_bcc**2)
n_fcc = np.sqrt(h_fcc**2 + k_fcc**2 + l_fcc**2)
...
for row in range(len(PeakAngle)-1):
    for column in range(len(n_bcc)):
        n_search_field[row+1, column] = n_search_field[row, column]/c_bcc[row]
...
for column in range(len(n_bcc)):
    ...
    for row in range(len(PeakAngle)-1):
        (n_best_field[row+1, column],
            n_res_field[row+1, column]) = find_best_fit(n_search_field[row+1,
                                                        column], n_bcc_help,
                                                        row+1)
        help_list = n_bcc_help.tolist()
